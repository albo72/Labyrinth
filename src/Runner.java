...
public class Runner {
    ...
    public List<LinkedList> moveAndGetTheWay(int x, int y, Labyrinth labyrinth,List<Coordinates> listOfExits, List<Coordinates> listOfPreviousCoordinates,LinkedList<Coordinates> steps, List<LinkedList> way) {
        ...
        if (labyrinth.getContent().get(y).get(x-1).equals('.') && !checker.checkPreviousSteps(x-1,y,listOfPreviousCoordinates)){
            if (!checker.compareWithExits(x-1,y,listOfExits)) {
        ...
        if (labyrinth.getContent().get(y+1).get(x).equals('.') && !checker.checkPreviousSteps(x,y+1,listOfPreviousCoordinates)){
            if (!checker.compareWithExits(x,y+1,listOfExits)) {
